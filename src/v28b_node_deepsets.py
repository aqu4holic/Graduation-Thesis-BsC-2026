"""
v28b_node_deepsets.py — ADIA Causal Discovery
  Node-centric 8ch 2D density + Deep Sets mean pooling (NO attention, NO edge pipeline)

Fix for v28 (collapsed to 12.5%): replace self-attention with mean pooling.

Per node v:
  8ch 32x32 image -> hierarchical conv2d -> node_emb (d=64)
Graph context:
  mean_pool(all valid node_embs, masked) -> graph_ctx (d=64)
Per node:
  [node_emb ; graph_ctx] -> MLP -> 8-class

Why mean pooling instead of attention:
  - v26c proved conv2d extracts 51% signal from images alone
  - v28 attention collapsed because it amplified noise before conv2d learned
  - Mean pooling is unconditionally stable: no softmax, no heads, no collapse
  - Still provides cross-node context: "the average node in this graph looks like X"
  - Deep Sets theory: mean pooling is a universal set function approximator

Channels (same as v28):
...
class ADIAModel(nn.Module):
    """v28b: Node-centric 8ch 2D + Deep Sets mean pooling.

    No attention. No edge pipeline. No conv1d.
    Just: conv2d per node -> mean pool context -> MLP classify.

    Each node gets [node_emb ; graph_context] where graph_context = mean(all node_embs).
    This is the Deep Sets / PointNet approach — provably a universal set function.
    """
    ...
    def __init__(self, d=None):
        ...
        # Per-node classification: [node_emb ; graph_ctx] -> 8-class
        # 2d input because we concat node_emb (d) + graph_ctx (d)
    ...
    def forward(self, node_imgs, node_mask=None, **kwargs):
        ...
        # Masked mean pooling for graph context
        if node_mask is not None:
            mask_f = node_mask.unsqueeze(-1).float()          # (B, K, 1)
            sum_embs = (node_embs * mask_f).sum(dim=1)        # (B, d)
            count = mask_f.sum(dim=1).clamp(min=1.0)          # (B, 1)
            graph_ctx = sum_embs / count                      # (B, d)
        else:
            graph_ctx = node_embs.mean(dim=1)                 # (B, d)

        # Expand graph context to match each node
        graph_ctx_exp = graph_ctx.unsqueeze(1).expand_as(node_embs)  # (B, K, d)

        # Concat [node_emb ; graph_ctx] and classify
        combined = torch.cat([node_embs, graph_ctx_exp], dim=-1)     # (B, K, 2d)
        all_logits = self.node_head(combined)                        # (B, K, N_CLASSES)
...
def train(X_train, y_train, model_directory_path):
    ...
    # Reuse v28 cached data — same preprocessing
    ...
    wandb_logger = WandbLogger(
        ...
        config={
            ...
            "edge_pipeline": False,
            "node_self_attention": False,
            "context_method": "mean_pool",
        },
    )
    ...
    path = os.path.join(model_directory_path, "model_v28b.pt")
...
if __name__ == "__main__":
    print("=" * 60)
    print("v28b: Node-Centric 8ch 2D + Deep Sets Mean Pooling")
    ...
    print(f"  Context: mean pool (Deep Sets) — NO attention")
    print(f"  Head: [node_emb ; graph_ctx] -> MLP -> 8-class")
    print(f"  NO edge pipeline, NO conv1d, NO O(p^2) preprocessing")
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ADIAModel(d=D_MODEL)
    model.load_state_dict(torch.load("resources/model_v28b.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv28b Balanced Accuracy: {np.mean(accs):.4f}")