"""
v19_consistency.py — ADIA Causal Discovery
  v11 architecture + edge-node consistency loss + binary decomposition

Key changes from v11:
  1. Consistency loss: BCE between edge_head's 4 diagnostic edge probs
     and the expected binary pattern from CLASS_EDGE_PATTERN[node_label].
     This gives the edge_head explicit supervision for the 4 key edges
     (v->X, v->Y, X->v, Y->v), calibrating it for binary decomposition.

  2. At inference: combine node_logits + alpha * binary_decomposition_scores.
     v18 showed alpha=1.0 gives +0.4% at inference-only; with consistency
     loss the edge probs should be far more accurate.

  Training loss: L_edge + L_node + lambda_consist * L_consistency
  Architecture: identical to v11 (2 attn layers, 6 struct types)
  Preprocessing: reuses v8b_anm_base cache

Usage:
    python v19_consistency.py
"""
...
# v19: Binary edge pattern for each class
...
ALPHA_INFER: float = 1.0  # binary decomposition weight at inference
...
class ADIAModel(nn.Module):
    """v19: v11 architecture (identical weights) + consistency-aware forward.

    No new parameters. The only change is that forward() now also returns
    the 4 diagnostic edge probabilities per node for the consistency loss.
    """
    ...
    def forward(
        ...
        alpha: float = 0.0,
        return_consistency_targets: bool = False,
    ) -> tuple:
        ...
        # v19: collect 4 diagnostic edge logits per node for consistency loss
        diag_edge_logits_list: list[torch.Tensor | None] = []
        ...
        for b in range(B):
            ...
            if not other:
                ...
                diag_edge_logits_list.append(None)
                continue

            node_logits_batch: list[torch.Tensor] = []
            diag_logits_batch: list[torch.Tensor] = []
            ...
            for v_name in other:
                ...
                emb_vx = edge_emb[b, idx_vx]
                emb_vy = edge_emb[b, idx_vy]
                emb_xv = edge_emb[b, idx_xv]
                emb_yv = edge_emb[b, idx_yv]

                # Standard node head
                node_emb = self.node_merge([emb_vx, emb_vy, emb_xv, emb_yv])
                logits = self.node_head(node_emb)

                # v19: Binary decomposition at inference
                if alpha > 0 and not self.training:
                    edge_probs_4 = torch.stack([
                        F.softmax(edge_logits[b, idx_vx], dim=-1)[1],
                        F.softmax(edge_logits[b, idx_vy], dim=-1)[1],
                        F.softmax(edge_logits[b, idx_xv], dim=-1)[1],
                        F.softmax(edge_logits[b, idx_yv], dim=-1)[1],
                    ])
                    p_edge = edge_probs_4.clamp(min=1e-6, max=1 - 1e-6)
                    log_p = torch.log(p_edge)
                    log_1mp = torch.log(1 - p_edge)
                    binary_scores = (pattern * log_p + (1 - pattern) * log_1mp).sum(dim=-1)
                    logits = logits + alpha * binary_scores

                node_logits_batch.append(logits)

                # v19: Collect diagnostic edge logits for consistency loss
                if return_consistency_targets:
                    diag = torch.stack([
                        edge_logits[b, idx_vx],  # (2,) logits for v->X
                        edge_logits[b, idx_vy],  # (2,) logits for v->Y
                        edge_logits[b, idx_xv],  # (2,) logits for X->v
                        edge_logits[b, idx_yv],  # (2,) logits for Y->v
                    ])  # (4, 2)
                    diag_logits_batch.append(diag)

            node_logits_list.append(
                torch.stack(node_logits_batch) if node_logits_batch else None
            )
            if return_consistency_targets and diag_logits_batch:
                diag_edge_logits_list.append(torch.stack(diag_logits_batch))
            else:
                diag_edge_logits_list.append(None)

        if return_consistency_targets:
            return edge_logits, node_logits_list, diag_edge_logits_list
...
class ADIAModelWrapper(pl.LightningModule):
    def __init__(
        ...
        # v19: BCE for consistency loss (no class weighting — each edge is binary)
        self.consistency_criterion = nn.BCEWithLogitsLoss(reduction='mean')

        # Register pattern as buffer so it moves to correct device
    ...
    def forward(self, batch: dict) -> tuple:
        return self.model(
            ...
            return_consistency_targets=True,
        )

    def _compute_loss(self, batch: dict, split: str) -> torch.Tensor:
        edge_logits, node_logits_list, diag_edge_logits_list = self(batch)
        ...
        # --- v19: Consistency loss ---
        if "node_labels" in batch and self.lambda_consistency > 0:
            all_diag_logits: list[torch.Tensor] = []
            all_diag_targets: list[torch.Tensor] = []
            nl = batch["node_labels"].to(self.device)

            for b in range(B):
                if diag_edge_logits_list[b] is not None:
                    K = diag_edge_logits_list[b].shape[0]
                    labels_b = nl[b, :K]
                    ...
                    for k in range(K):
                        ...
                        if lbl < 0:
                            continue
                        # diag_edge_logits_list[b][k] is (4, 2) — logits for 4 edges
                        # We want the logit for class 1 (edge present)
                        diag_logit = diag_edge_logits_list[b][k, :, 1]  # (4,)
                        # Target: CLASS_EDGE_PATTERN[label] — (4,) binary
                        target = self.class_edge_pattern[lbl]  # (4,)
                        all_diag_logits.append(diag_logit)
                        all_diag_targets.append(target)

            if all_diag_logits:
                cat_diag_logits = torch.stack(all_diag_logits)   # (N_nodes, 4)
                cat_diag_targets = torch.stack(all_diag_targets)  # (N_nodes, 4)
                consist_loss = self.consistency_criterion(cat_diag_logits, cat_diag_targets)
                total_loss = total_loss + self.lambda_consistency * consist_loss
                n_terms += 1
                self.log(f"{split}/consist_loss", consist_loss, prog_bar=True, sync_dist=True)
...
def train(
    ...
    print(f"Consistency loss lambda: {LAMBDA_CONSISTENCY}")
    ...
    trainer.fit(wrapper, loader, ckpt_path="/data/anhld48/phatnt/Graduation-Thesis-BsC-2026/src/lightning_logs/version_63/checkpoints/epoch=8-step=3312.ckpt")

    path: str = os.path.join(model_directory_path, "model_v19.pt")
...
@torch.no_grad()
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None, alpha=1.0):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v19_nk{N_KERNEL}.pkl") if cache_dir else None
    ...
    for batch in tqdm(loader, desc=f"inferring (alpha={alpha})"):
        edge_logits, node_logits_list = model(
            ...
            alpha=alpha,
            return_consistency_targets=False)
...
def infer(X_test, model_directory_path, id_column_name, prediction_column_name):
    ...
    adj_list = infer_batch_local(dfs, model, device=device, batch_size=64,
                                  cache_dir=cache_dir, alpha=ALPHA_INFER)
    submission = {}
...
if __name__ == "__main__":
    print("=" * 60)
    print("v19: Edge-Node Consistency Loss + Binary Decomposition")
    print(f"  lambda_consistency={LAMBDA_CONSISTENCY}, alpha_infer={ALPHA_INFER}")
    print(f"  Same architecture as v11 (2 attn layers, 6 struct types)")
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v19.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\n{'alpha':>6s} | {'BalAcc':>7s} | {'Conf':>6s} {'Coll':>6s} {'Med':>6s} {'CauX':>6s} {'CauY':>6s} {'CnqX':>6s} {'CnqY':>6s} {'Indep':>6s}")
    print("-" * 90)
    ...
    print(f"\nDone. Delete resources/model.pt and infer cache before running.")