"""
v11b_structbias.py — ADIA Causal Discovery
  v8b + X/Y-aware Structural Attention Bias

Upgrade from v11: chain types split by whether the shared node is X/Y
or a regular variable. Directly targets Mediator confusion.

8 structural relationship types:
  Type 0: Reverse pair
  Type 1: Shared source
  Type 2: Shared target
  Type 3: Fwd chain through X/Y   (shared node in {X,Y})
  Type 4: Fwd chain through other  (shared node not in {X,Y}) — MEDIATOR
  Type 5: Bwd chain through X/Y
  Type 6: Bwd chain through other
  Type 7: Unrelated

Base 25K go/no-go. Target: beat v11 base 79.59%.

Usage:
    python v11b_structbias.py
"""
...
D_MODEL: int = 64
N_HEADS: int = 4
N_STRUCT_TYPES: int = 8  # X/Y-aware structural relationship types
...
# ============================================================
# Structural Relationship Matrix — X/Y-aware
# ============================================================
def build_struct_rel_matrix(p: int, xi: int, yi: int) -> np.ndarray:
    """Build E×E matrix of X/Y-aware structural relationship types.

    Types:
      0: Reverse pair     (u1=v2 AND v1=u2)
      1: Shared source    (u1=u2, not reverse)
      2: Shared target    (v1=v2, not reverse)
      3: Fwd chain through X/Y   (v1=u2, shared node ∈ {xi,yi})
      4: Fwd chain through other  (v1=u2, shared node ∉ {xi,yi}) — MEDIATOR chain
      5: Bwd chain through X/Y   (u1=v2, shared node ∈ {xi,yi})
      6: Bwd chain through other  (u1=v2, shared node ∉ {xi,yi})
      7: Unrelated

    Returns: (E, E) int64 array where E = p*(p-1)
    """
    E: int = p * (p - 1)
    xy_set: set[int] = {xi, yi}
    ...
    rel: np.ndarray = np.full((E, E), 7, dtype=np.int64)  # default: unrelated
    ...
    for e1 in range(E):
        ...
        for e2 in range(E):
            ...
            if u1 == v2 and v1 == u2:
                ...
                # forward chain — shared node is v1 (= u2)
                if v1 in xy_set:
                    rel[e1, e2] = 3  # chain through X/Y
                else:
                    rel[e1, e2] = 4  # chain through other (mediator-like)
            elif u1 == v2:
                # backward chain — shared node is u1 (= v2)
                if u1 in xy_set:
                    rel[e1, e2] = 5  # chain through X/Y
                else:
                    rel[e1, e2] = 6  # chain through other

    return rel


# Cache by (p, xi, yi) since X/Y position varies per graph
_STRUCT_REL_CACHE: dict[tuple[int, int, int], np.ndarray] = {}


def get_struct_rel_matrix(p: int, xi: int, yi: int) -> np.ndarray:
    global _STRUCT_REL_CACHE
    key: tuple = (p, xi, yi)
    if key not in _STRUCT_REL_CACHE:
        _STRUCT_REL_CACHE[key] = build_struct_rel_matrix(p, xi, yi)
...
def collate_fn(batch: list[dict]) -> dict:
    ...
    # Default to type 7 (unrelated) for padding
    struct_rel: torch.Tensor = torch.full(
        (B, max_E, max_E), 7, dtype=torch.long
    )
    ...
    for b, item in enumerate(batch):
        ...
        # Build X/Y-aware struct rel matrix
        p: int = item["p"]
        cols: list[str] = item["cols"]
        xi: int = cols.index("X")
        yi: int = cols.index("Y")
        rel_mat: np.ndarray = get_struct_rel_matrix(p, xi, yi)
    ...
    def forward(
        ...
        if struct_rel is None:
            struct_rel = torch.full(
                (B, E, E), 7, dtype=torch.long, device=edge_data.device
            )
            for b in range(B):
                cols_b: list[str] = cols_list[b]
                p: int = len(cols_b)
                e: int = p * (p - 1)
                xi: int = cols_b.index("X")
                yi: int = cols_b.index("Y")
                rel: np.ndarray = get_struct_rel_matrix(p, xi, yi)
...
if __name__ == "__main__":
    print("=" * 60)
    print("v11b X/Y-aware Structural Attention Bias")
    print(f"  8 struct types (chain split by X/Y), {N_HEADS} heads, d={D_MODEL}")
