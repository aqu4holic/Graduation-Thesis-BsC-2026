"""
v21_partial_resid.py — ADIA Causal Discovery
  v11 + XY-partial residual channels (new preprocessing)

New preprocessing channels that capture 3-way (X, Y, v) relationships:

  For each graph, compute:
    resid_Y_given_X = Y - E[Y|X]   (pairwise kernel regression, bw=0.5)
    resid_X_given_Y = X - E[X|Y]

  Add 2 channels to EVERY edge (i->j):
    ch9:  resid_Y_given_X sorted by source i
    ch10: resid_X_given_Y sorted by source i

  N_CHANNELS: 8 -> 10

Why this helps Mediator detection:
  For edge v->Y with Mediator (X->v->Y):
    resid_Y_given_X sorted by v shows CLEAR PATTERN
    (v's effect on Y survives after removing X's direct effect)
  For edge v->Y with ConseqX (X->v only):
    resid_Y_given_X sorted by v is FLAT NOISE
    (v doesn't cause Y, removing X removes all signal)

  This DIRECTLY distinguishes Mediator from ConseqX — the #1 confusion.

Current ANM residuals regress out ALL variables (including v), which is
too aggressive — it removes v's mediating effect. XY-partial residuals
remove ONLY the X->Y direct path, preserving mediated effects.

Same architecture as v11. Only preprocessing changes.
REQUIRES NEW CACHE — cannot reuse v8b_anm_base cache.

Usage:
    python v21_partial_resid.py
"""
...
N_CHANNELS: int = 2 + len(BANDWIDTHS) + len(BANDWIDTHS) + 2  # 10 (was 8)
...
# ============================================================
# Data Preprocessing — v11 + XY-partial residuals
# ============================================================
...
def compute_pairwise_residual(
    source: np.ndarray, target: np.ndarray, sub_idx: np.ndarray,
    bandwidth: float = 0.5,
) -> np.ndarray:
    """Compute target - E[target|source] using 1D kernel regression.

    Returns residual array of shape (N,) as float32.
    """
    N: int = len(source)
    n_sub: int = len(sub_idx)
    ...
    # 1D kernel weights
    diff: np.ndarray = source[:, None] - source_sub[None, :]  # (N, n_sub)
    W: np.ndarray = np.exp(-diff ** 2 / (2 * bandwidth ** 2))  # (N, n_sub)
    W_sum: np.ndarray = W.sum(axis=1, keepdims=True) + 1e-10  # (N, 1)
    W_norm: np.ndarray = W / W_sum  # (N, n_sub)

    # E[target|source] = weighted average of target_sub
    target_hat: np.ndarray = W_norm @ target_sub  # (N,)
...
def build_edge_tensor(
    ...
    # Standard multivariate kernel regression
    ...
    # v21: Compute XY-partial residuals
    x_col_idx: int = cols.index("X") if "X" in cols else 0
    y_col_idx: int = cols.index("Y") if "Y" in cols else 1

    x_data: np.ndarray = data[:, x_col_idx]
    y_data: np.ndarray = data[:, y_col_idx]

    # resid_Y_given_X = Y - E[Y|X] at bw=0.5
    resid_y_given_x: np.ndarray = compute_pairwise_residual(
        x_data, y_data, sub_idx, bandwidth=0.5
    )
    # resid_X_given_Y = X - E[X|Y] at bw=0.5
    resid_x_given_y: np.ndarray = compute_pairwise_residual(
        y_data, x_data, sub_idx, bandwidth=0.5
    )
    ...
    for i, u_name in enumerate(cols):
        ...
        for j, v_name in enumerate(cols):
            ...
            # v21: XY-partial residual channels sorted by source
            partial_y_channel: np.ndarray = resid_y_given_x[sort_idx]
            partial_x_channel: np.ndarray = resid_x_given_y[sort_idx]

            channels: list[np.ndarray] = (
                [u_sorted, v_sorted_by_u]
                + kernel_channels
                + anm_channels
                + [partial_y_channel, partial_x_channel]
            )
...
# ============================================================
# Model Architecture (identical to v11, just N_CHANNELS=10)
# ============================================================
...
class ADIAModel(nn.Module):
    """v21: v11 architecture with N_CHANNELS=10 (was 8)."""
...
def train(
    ...
    # v21: NEW CACHE — cannot reuse v8b cache (different channels)
    cache_path: str = os.path.join(
        LOCAL_CACHE_DIR, f"train_dataset_v21_partial_nk{N_KERNEL}.pkl"
    )
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    # v21: different cache name
    cache_path = os.path.join(cache_dir, f"infer_v21_partial_nk{N_KERNEL}.pkl") if cache_dir else None
...
if __name__ == "__main__":
    print("=" * 60)
    print("v21: XY-Partial Residual Channels")
    print(f"  N_CHANNELS: 10 (was 8) — 2 new partial residual channels")
    print(f"  resid_Y_given_X and resid_X_given_Y sorted per edge")
    print(f"  Config: bs={BATCH_SIZE}, lr={LR}, epochs={MAX_EPOCHS}")
    print(f"  REQUIRES NEW CACHE (cannot reuse v8b cache)")
    print("=" * 60)
