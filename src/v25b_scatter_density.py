"""
v25b_scatter_density.py — ADIA Causal Discovery
...
Usage:
    python v25b_scatter_density.py
"""
...
# ============================================================
# 2D Preprocessing — scatter density images (v25b: NO rank, NO row-norm)
# ============================================================
...
class ADIAModel(nn.Module):
    """v25b: Edge-level 1D pipeline (edge_head) + node-level 2D pipeline FUSED with edges (node_head).
    ...
    with NO context from edges. v25b merges node_2d_emb with 4 edge embeddings.
    """
    ...
    def __init__(self, d=None, n_edge_types=None, aug_noise_std=0.0):
        ...
        # Node pipeline (v25b: fused with edge embeddings)
...
def train(X_train, y_train, model_directory_path):
    ...
    cache_path = os.path.join(LOCAL_CACHE_DIR, f"train_dataset_v25b_scatter_nk{N_KERNEL}.pkl")
    ...
    path = os.path.join(model_directory_path, "model_v25b.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v25b_nk{N_KERNEL}.pkl") if cache_dir else None
...
if __name__ == "__main__":
    print("=" * 60)
    print("v25b: Scatter Density 2D + Edge-Fused Node Classification")
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v25b.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv25b Balanced Accuracy: {np.mean(accs):.4f}")
