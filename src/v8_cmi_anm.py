"""
v8_cmi_anm.py — ADIA Causal Discovery
  Multi-bandwidth kernel regression (5ch) + Local CMI (3ch) + ANM residuals (3ch)
  = 11 input channels per edge
...
Channels per edge (u→v):
  ...
  Ch 8:  ANM multivariate residual (v - ŷ_v) at bandwidth=0.2, sorted by u
  Ch 9:  ANM multivariate residual (v - ŷ_v) at bandwidth=0.5, sorted by u
  Ch 10: ANM multivariate residual (v - ŷ_v) at bandwidth=1.0, sorted by u

Insight: Under correct causal direction u→v, the residual v - E[v|all] should be
independent of u (flat pattern when sorted by u). Under wrong direction, residuals
correlate with the predictor (structured pattern). Reuses existing kernel regression
coefficients — zero additional kernel computation.

Architecture: v2 baseline with N_CHANNELS=11, no StatProjector, no focal loss.
Training: base 25K, bs=16, lr=1e-3, 30 epochs, 2 GPU DDP.

Usage:
    python v8_cmi_anm.py
"""
...
# Total channels: 2 (sorted obs) + 3 (kernel bw) + 3 (CMI k) + 3 (ANM resid bw) = 11
N_CHANNELS: int = 2 + len(BANDWIDTHS) + len(CMI_K_VALUES) + len(BANDWIDTHS)
...
def build_edge_tensor(
    ...
    """
    Build 11-channel edge tensor: multi-bandwidth kernel + local CMI + ANM residuals.

    Channels per edge (u→v):
      Ch 0:  sorted u observations
      Ch 1:  v observations sorted by u
      Ch 2-4:  kernel regression coeff at bw 0.2, 0.5, 1.0
      Ch 5-7:  local CMI contribution at k=3, 5, 10
      Ch 8-10: ANM multivariate residual (v - ŷ_v) at bw 0.2, 0.5, 1.0
    ...
    for i, u_name in enumerate(cols):
        ...
        for j, v_name in enumerate(cols):
            ...
            # ANM residual channels: residual of predicting v from all others,
            # at each bandwidth, sorted by u.
            # Under correct direction u→v, residuals ≈ flat noise when sorted by u.
            # Under wrong direction, residuals have structure.
            ...
            edge_tensor: np.ndarray = np.stack(channels, axis=0)  # (11, N)
...
# ============================================================
# Model Architecture — v2 baseline with N_CHANNELS=11
# ============================================================
...
class ADIAModel(nn.Module):
    """
    v8: v2 baseline architecture with 11-channel input.
...
def train(
    ...
    # Class weights
    node_w_path: str = os.path.join(LOCAL_CACHE_DIR, "node_weights_v8.pt")
    edge_w_path: str = os.path.join(LOCAL_CACHE_DIR, "edge_weights_v8.pt")
    ...
    cache_path: str = os.path.join(
        LOCAL_CACHE_DIR, f"train_dataset_v8_cmi_anm_base_nk{N_KERNEL}.pkl"
    )
    ...
    trainer.fit(wrapper, loader, ckpt_path='lightning_logs/version_17/checkpoints/epoch=14-step=22035.ckpt')
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v8_nk{N_KERNEL}.pkl") if cache_dir else None
...
if __name__ == "__main__":
    print("=" * 60)
    print("v8_cmi_anm: Multi-bandwidth + Local CMI + ANM residuals")
    print(f"Channels: {N_CHANNELS} (2 sorted + {len(BANDWIDTHS)} kernel + {len(CMI_K_VALUES)} CMI + {len(BANDWIDTHS)} ANM)")
