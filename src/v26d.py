"""
v26d_density8ch.py — ADIA Causal Discovery
...
class ADIAModel(nn.Module):
    """v26d: Edge 1D pipeline + 8ch node 2D pipeline FUSED with edge embeddings.
    ...
    def __init__(self, d=None, n_edge_types=None, aug_noise_std=0.0):
        ...
        # Node pipeline (v26d: 8ch density fused with edges)
    ...
    def _compute_loss(self, batch, split):
        ...
        if "edge_labels" in batch:
            ...
            if fl.numel() > 0:
                ...
                total_loss = total_loss + 0.3 * edge_loss
                n_terms += 1
...
def train(X_train, y_train, model_directory_path):
    ...
    cache_path = os.path.join(LOCAL_CACHE_DIR, f"train_dataset_v26d_noch{N_NODE_CHANNELS}_nk{N_KERNEL}_ss{SCATTER_SIGMA}_ss{ANM_SIGMA}.pkl")
    ...
    path = os.path.join(model_directory_path, "model_v26d.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v26d_noch{N_NODE_CHANNELS}_nk{N_KERNEL}_ss{SCATTER_SIGMA}_ss{ANM_SIGMA}.pkl") if cache_dir else None
...
if __name__ == "__main__":
    ...
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v26d.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
