"""
v24b_pure_node2d.py — ADIA Causal Discovery
  PURE node-centric 4-channel 2D conditional density (NO edge pipeline)

Same 4-channel representation as v24:
  ch0: P(rank_X | rank_v), ch1: P(rank_Y | rank_v)
  ch2: P(rank_v | rank_X), ch3: P(rank_v | rank_Y)

NO edge pipeline — no conv1d, no self-attention, no edge_head.
Each node classified purely from its 4-channel density image.
Dramatically simpler: just Conv2d → pool → classify.

Tests whether the 2D triplet representation is self-sufficient.

Usage:
    python v24b_pure_node2d.py
"""
...
class ADIAModel(nn.Module):
    """v24b: PURE node-centric 2D. No edge pipeline at all.

    Only: conv2d on 4-channel density images → node_head (8-class)
    Node predictions directly set the adjacency matrix edges.
    """
    ...
    def forward(self, node_imgs, node_img_mask=None, **kwargs):
        """Pure node-centric forward. Ignores edge_data etc.

        Args:
            node_imgs: (B, K, 4, H, W)
            node_img_mask: (B, K) bool
        Returns:
            node_logits_list: list of (K_b, 8) tensors
        """
        B, K, C, H, W = node_imgs.shape

        flat_imgs = node_imgs.view(B * K, C, H, W)
        flat_emb = self.node_extractor(flat_imgs)
        flat_logits = self.node_head(flat_emb)
        all_logits = flat_logits.view(B, K, N_CLASSES)
...
def train(X_train, y_train, model_directory_path):
    ...
    path = os.path.join(model_directory_path, "model_v24b.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v24b_nk{N_KERNEL}.pkl") if cache_dir else None
    ...
    for batch in tqdm(loader, desc="inferring"):
        ...
        for b in range(B_cur):
            ...
            # Build adjacency purely from node predictions
...
if __name__ == "__main__":
    print("=" * 60)
    print("v24b: PURE Node-Centric 2D Density (no edge pipeline)")
    print(f"  Node: 4ch x {GRID_SIZE}x{GRID_SIZE} conditional densities")
    print(f"  No conv1d, no self-attention, no edge_head")
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v24b.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv24b Balanced Accuracy: {np.mean(accs):.4f}")