"""
v20_multiply.py — ADIA Causal Discovery
  v11 + multiplicative edge interactions in node_merge

Single change from v11: the node head now receives 6 inputs instead of 4.
The 2 extra inputs are element-wise products of edge embeddings that
capture conjunctive causal patterns:

  emb_xv * emb_vy  →  "X→v AND v→Y both active" = Mediator signal
  emb_vx * emb_vy  →  "v→X AND v→Y both active" = Confounder signal

The node_merge Linear grows from 4*64=256 to 6*64=384 input dims.
Everything else is identical to v11 (same conv, same attention, same edge head).

Motivation: v11's node_merge is Linear(256,64) — it can only learn LINEAR
combinations of the 4 embeddings. It cannot learn "are BOTH X→v and v→Y
active?" because that requires a multiplicative interaction. Element-wise
products provide this conjunction feature directly.

This is NOT a new module — just 2 extra terms in a concatenation. ~10K
extra params (384*64+64 vs 256*64+64 in merge Linear).

Usage:
    python v20_multiply.py
"""
...
# ============================================================
# Model Architecture — v11 + multiplicative interactions
# ============================================================
...
class ADIAModel(nn.Module):
    """v20: v11 + multiplicative edge interactions in node_merge.

    Only change: node_merge takes 6 inputs instead of 4.
    Extra inputs are element-wise products capturing conjunction patterns.
    """
    ...
    def __init__(self, d: int = None, n_edge_types: int = None, aug_noise_std: float = 0.0):
        ...
        # v20: 6 inputs instead of 4 — adds 2 multiplicative interactions
        self.node_merge = MergeOperator(n_inputs=6, d=d)
    ...
    def forward(
        ...
        for b in range(B):
            ...
            for v_name in other:
                ...
                emb_vx = edge_emb[b, _eidx(vi, xi)]  # v→X
                emb_vy = edge_emb[b, _eidx(vi, yi)]  # v→Y
                emb_xv = edge_emb[b, _eidx(xi, vi)]  # X→v
                emb_yv = edge_emb[b, _eidx(yi, vi)]  # Y→v

                # v20: multiplicative interactions
                mediator_signal = emb_xv * emb_vy    # X→v AND v→Y
                confounder_signal = emb_vx * emb_vy   # v→X AND v→Y

                node_emb = self.node_merge([
                    emb_vx, emb_vy, emb_xv, emb_yv,
                    mediator_signal, confounder_signal,
                ])
...
if __name__ == "__main__":
    print("=" * 60)
    print("v20: Multiplicative Edge Interactions in Node Merge")
    print(f"  6-input merge: [vX, vY, Xv, Yv, Xv*vY, vX*vY]")
