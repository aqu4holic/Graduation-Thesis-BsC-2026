"""
v17_xyaware_tta.py — ADIA Causal Discovery
  Tier 1 improvements over v11: X/Y-aware structural bias + test-time augmentation

Changes from v11:
  1. Structural bias: 6 → 10 types, X/Y-aware chain detection
     - Type 3: X→v→Y mediator chain (fixes 57% of Mediator errors)
     - Type 4: Y→v→X reverse mediator
     - Type 5-6: Partial chains through X or Y
     - Type 7-8: Other forward/backward chains

  2. Test-time augmentation: infer with (X,Y) and (Y,X), average probabilities
     - Applies label swapping: CauseX ↔ CauseY, ConseqX ↔ ConseqY
     - Expected gain: +0.3-0.5% from symmetry exploitation

Expected improvement: +1.5-2.5% over v11 base (79.59% → ~81-82%)

Usage:
    python v17_xyaware_tta.py
"""
...
D_MODEL: int = 64
N_HEADS: int = 4
N_STRUCT_TYPES: int = 10  # v17: 10 types (was 6 in v11)
...
# Label swap mapping for XY augmentation at test time
XY_SWAP_LABEL_MAP: dict[int, int] = {
    0: 0,  # Confounder → Confounder
    1: 1,  # Collider → Collider
    2: 2,  # Mediator → Mediator
    3: 4,  # Cause of X → Cause of Y
    4: 3,  # Cause of Y → Cause of X
    5: 6,  # Consequence of X → Consequence of Y
    6: 5,  # Consequence of Y → Consequence of X
    7: 7,  # Independent → Independent
}
...
# ============================================================
# v17: X/Y-Aware Structural Relationship Matrix
# ============================================================
def build_struct_rel_matrix(p: int, x_idx: int, y_idx: int) -> np.ndarray:
    """Build E×E matrix of structural relationship types for a p-node graph.

    v17: X/Y-aware chain types to fix Mediator half-chain detection failures.

    Types:
      0: Reverse pair     (u1=v2 AND v1=u2)
      1: Shared source    (u1=u2, not reverse)
      2: Shared target    (v1=v2, not reverse)
      3: X→v→Y mediator   (u1=X, v1=u2, v2=Y) — KEY FIX
      4: Y→v→X reverse    (u1=Y, v1=u2, v2=X) — KEY FIX
      5: Fwd chain thru X (v1=u2=X, not 3/4)
      6: Fwd chain thru Y (v1=u2=Y, not 3/4)
      7: Other fwd chain  (v1=u2, not 0,3-6)
      8: Backward chain   (u1=v2, not 0)
      9: Unrelated
    ...
    rel: np.ndarray = np.full((E, E), 9, dtype=np.int64)  # default: unrelated
    ...
    for e1 in range(E):
        ...
        for e2 in range(E):
            ...
            # Type 1: Shared source
            if u1 == u2:
                rel[e1, e2] = 1
                continue

            # Type 2: Shared target
            if v1 == v2:
                rel[e1, e2] = 2
                continue

            # === v17 NEW: X/Y-aware chain types ===

            # Type 3: X→v→Y mediator chain (fixes 57% of Mediator errors)
            if u1 == x_idx and v2 == y_idx and v1 == u2:
                rel[e1, e2] = 3
                continue

            # Type 4: Y→v→X reverse mediator
            if u1 == y_idx and v2 == x_idx and v1 == u2:
                rel[e1, e2] = 4
                continue

            # Type 5: Forward chain through X (but not full X→Y mediation)
            if v1 == x_idx and u2 == x_idx:
                rel[e1, e2] = 5
                continue

            # Type 6: Forward chain through Y (but not full Y→X mediation)
            if v1 == y_idx and u2 == y_idx:
                rel[e1, e2] = 6
                continue

            # Type 7: Other forward chain
            if v1 == u2:
                rel[e1, e2] = 7
                continue

            # Type 8: Backward chain
            if u1 == v2:
                rel[e1, e2] = 8
                continue

            # Type 9: Unrelated (default, already set)

    return rel


# Cache struct rel matrices per (p, x_idx, y_idx)
_STRUCT_REL_CACHE: dict[tuple[int, int, int], np.ndarray] = {}


def get_struct_rel_matrix(p: int, x_idx: int, y_idx: int) -> np.ndarray:
    global _STRUCT_REL_CACHE
    key: tuple = (p, x_idx, y_idx)
    if key not in _STRUCT_REL_CACHE:
        _STRUCT_REL_CACHE[key] = build_struct_rel_matrix(p, x_idx, y_idx)
    return _STRUCT_REL_CACHE[key]
...
def collate_fn(batch: list[dict]) -> dict:
    ...
    # v17: struct_rel now depends on (p, x_idx, y_idx)
    struct_rel: torch.Tensor = torch.full(
        (B, max_E, max_E), 9, dtype=torch.long
    )
    ...
    for b, item in enumerate(batch):
        ...
        edge_mask[b, :E] = True
        cols: list[str] = item["cols"]
        cols_list.append(cols)

        # v17: find X, Y indices for this graph
        p: int = item["p"]
        x_idx: int = cols.index("X") if "X" in cols else 0
        y_idx: int = cols.index("Y") if "Y" in cols else 1
        rel_mat: np.ndarray = get_struct_rel_matrix(p, x_idx, y_idx)
...
# ============================================================
# Model Architecture — v11 + 10 structural types
# ============================================================
...
class StructuralSelfAttention(nn.Module):
    ...
    v17: Now supports 10 structural types (was 6 in v11).
    """

    def __init__(
        self, d: int = 64, n_heads: int = 4,
        n_struct_types: int = 10,
    ):
        super().__init__()
        self.d: int = d
        ...
        # v17: 10 types × 4 heads = 40 learnable params
...
class ADIAModel(nn.Module):
    """v17: v11 architecture + 10-type X/Y-aware structural bias."""
    ...
    def forward(
        ...
        if struct_rel is None:
            struct_rel = torch.full(
                (B, E, E), 9, dtype=torch.long, device=edge_data.device
            )
            for b in range(B):
                ...
                x_idx: int = cols.index("X") if "X" in cols else 0
                y_idx: int = cols.index("Y") if "Y" in cols else 1
                rel: np.ndarray = get_struct_rel_matrix(p, x_idx, y_idx)
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None, use_tta=True):
    """v17: Inference with optional test-time augmentation (XY swap)."""
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v17_nk{N_KERNEL}.pkl") if cache_dir else None
    ...
    # Store probabilities for TTA averaging
    all_node_probs = [None] * len(all_items)

    idx_off = 0
    for batch in tqdm(loader, desc="inferring (pass 1)"):
        ...
        for b in range(edge_logits.shape[0]):
            if node_logits_list[b] is not None:
                ...
                all_node_probs[idx_off + b] = torch.softmax(node_logits_list[b], dim=-1).cpu()
        idx_off += edge_logits.shape[0]

    # v17 TTA: Infer with X and Y swapped
    if use_tta:
        print("Applying test-time augmentation (XY swap)...")
        swapped_items = []
        for item in all_items:
            ...
            if "X" in cols and "Y" in cols:
                x_idx = cols.index("X")
                y_idx = cols.index("Y")
                cols[x_idx], cols[y_idx] = "Y", "X"
            swapped_item = item.copy()
            swapped_item["cols"] = cols
            swapped_items.append(swapped_item)

        loader_swap = DataLoader(InMemoryDataset(swapped_items), batch_size=batch_size,
                                 shuffle=False, num_workers=0, collate_fn=collate_fn)

        idx_off = 0
        for batch in tqdm(loader_swap, desc="inferring (pass 2, swapped)"):
            edge_logits, node_logits_list = model(
                batch["edge_data"].to(device), batch["edge_types"].to(device),
                batch["edge_mask"].to(device), batch["cols"],
                struct_rel=batch["struct_rel"].to(device))
            ...
            for b in range(edge_logits.shape[0]):
                if node_logits_list[b] is not None and all_node_probs[idx_off + b] is not None:
                    probs_swap = torch.softmax(node_logits_list[b], dim=-1).cpu()
                    # Apply label swap mapping
                    probs_swap_remapped = torch.zeros_like(probs_swap)
                    for orig_idx, swap_idx in XY_SWAP_LABEL_MAP.items():
                        probs_swap_remapped[:, orig_idx] = probs_swap[:, swap_idx]
                    # Average with original
                    all_node_probs[idx_off + b] = (all_node_probs[idx_off + b] + probs_swap_remapped) / 2.0
            idx_off += edge_logits.shape[0]

    # Convert probabilities to predictions and build adjacency matrices
    results = [None] * len(all_items)
    idx_off = 0
    loader_final = DataLoader(InMemoryDataset(all_items), batch_size=batch_size,
                              shuffle=False, num_workers=0, collate_fn=collate_fn)

    for batch in tqdm(loader_final, desc="building DAGs"):
        edge_logits, _ = model(
            batch["edge_data"].to(device), batch["edge_types"].to(device),
            batch["edge_mask"].to(device), batch["cols"],
            struct_rel=batch["struct_rel"].to(device))
        ...
        for b in range(edge_logits.shape[0]):
            ...
            if all_node_probs[idx_off + b] is not None:
                other_nodes = [n for n in cols if n not in ("X", "Y")]
                node_preds = torch.argmax(all_node_probs[idx_off + b], dim=-1)
...
if __name__ == "__main__":
    print("=" * 60)
    print("v17 X/Y-Aware Structural Bias + Test-Time Augmentation")
    print(f"  10 struct types (was 6), {N_HEADS} heads, d={D_MODEL}")
    print(f"  TTA: infer with (X,Y) and (Y,X), average probabilities")
    ...
    print(f"\nBalanced Accuracy (with TTA): {np.mean(accs):.4f}")