"""
v30_density8ch.py — ADIA Causal Discovery
  8-channel 2D scatter density (4 raw + 4 ANM) + edge context encoder

Based on v25a (79.56% BA) which proved 4ch density + edge fusion works.
Adds 4 ANM residual scatter channels with SEPARATE sigma to preserve
fine directional asymmetry that v26b (79.17%) lost due to over-smoothing.

For each node v, create an 8-channel 32x32 image:
  ch0: density(v, X)       — raw joint scatter (sigma=5.0)
  ch1: density(v, Y)       — raw joint scatter (sigma=5.0)
  ch2: density(X, v)       — transpose view    (sigma=5.0)
  ch3: density(Y, v)       — transpose view    (sigma=5.0)
  ch4: density(v, resid_X) — ANM residual of X (sigma=2.0, bw=0.5)
  ch5: density(X, resid_v) — ANM residual of v (sigma=2.0, bw=0.5)
  ch6: density(v, resid_Y) — ANM residual of Y (sigma=2.0, bw=0.5)
  ch7: density(Y, resid_v) — ANM residual of v (sigma=2.0, bw=0.5)

Key: dual sigma — raw channels use sigma=5.0 (coarse shape),
ANM channels use sigma=2.0 (preserve fine asymmetry between ch4 vs ch5).

Architecture: v11 edge pipeline as context encoder (edge loss = auxiliary),
hierarchical conv2d on 8ch images fused with 4 edge embeddings → 8-class.

Usage:
    python v30_density8ch.py
"""
...
N_KERNEL: int = 1000
GRID_SIZE: int = 32
N_NODE_CHANNELS: int = 8  # 4 raw density + 4 ANM residual density
RAW_SIGMA: float = 5.0    # Gaussian sigma for raw density channels
ANM_SIGMA: float = 2.0    # Gaussian sigma for ANM channels (finer)
ANM_BW: float = 0.5       # Bandwidth for ANM residual computation
N_CLASSES: int = 8
N_EDGE_TYPES: int = 7
D_MODEL: int = 64
N_HEADS: int = 4
...
# ============================================================
# 2D Preprocessing — 8-channel scatter density images (dual sigma)
# ============================================================
def build_scatter_density(source, target, grid_size=32, sigma=5.0):
    """Render scatter plot as grid_size x grid_size density image.
    Uses min-max normalization. Returns joint density."""
...
def build_node_images(df, resid_map=None):
    ...
    For each node v, returns (8, GRID_SIZE, GRID_SIZE):
      ch0: density(v, X)       — raw joint scatter   (sigma=RAW_SIGMA)
      ch1: density(v, Y)       — raw joint scatter   (sigma=RAW_SIGMA)
      ch2: density(X, v)       — transpose view      (sigma=RAW_SIGMA)
      ch3: density(Y, v)       — transpose view      (sigma=RAW_SIGMA)
      ch4: density(v, resid_X) — ANM residual of X   (sigma=ANM_SIGMA)
      ch5: density(X, resid_v) — ANM residual of v   (sigma=ANM_SIGMA)
      ch6: density(v, resid_Y) — ANM residual of Y   (sigma=ANM_SIGMA)
      ch7: density(Y, resid_v) — ANM residual of v   (sigma=ANM_SIGMA)

    Dual sigma: raw channels use RAW_SIGMA=5.0 for coarse shape,
    ANM channels use ANM_SIGMA=2.0 to preserve fine directional asymmetry.
    """
    cols = list(df.columns)
    data = df.values.astype(np.float32)
    ...
    for v_name in other_nodes:
        ...
        # Raw joint density channels (coarse sigma)
        ch0 = build_scatter_density(v_data, x_data, GRID_SIZE, sigma=RAW_SIGMA)
        ch1 = build_scatter_density(v_data, y_data, GRID_SIZE, sigma=RAW_SIGMA)
        ch2 = build_scatter_density(x_data, v_data, GRID_SIZE, sigma=RAW_SIGMA)
        ch3 = build_scatter_density(y_data, v_data, GRID_SIZE, sigma=RAW_SIGMA)

        # ANM residual density channels (fine sigma)
...
def _build_single(args):
    ...
    # Compute ANM residuals once, reuse for node images
    _, resid_map_05 = compute_multivariate_kernel_coefficients(data, sub_idx, bandwidth=ANM_BW)

    edge_data, edge_types = build_edge_tensor(df)
    node_images = build_node_images(df, resid_map=resid_map_05)
...
class ADIAModel(nn.Module):
    """v30: Edge 1D context encoder + 8ch node 2D pipeline fused with edge embeddings.

    Edge pipeline: v11 (conv1d -> struct attn -> edge_head as auxiliary loss)
    Node pipeline: hierarchical conv2d on 8ch density images (dual sigma)
    ...
    def __init__(self, d=None, n_edge_types=None, aug_noise_std=0.0):
        ...
        # Edge pipeline (context encoder, v11 architecture)
        ...
        self.edge_head = nn.Linear(d, 2)  # auxiliary loss only

        # Node pipeline (8ch density fused with edge context)
...
class ADIAModelWrapper(pl.LightningModule):
    ...
    def _compute_loss(self, batch, split):
        ...
        # Edge loss (auxiliary — representation learning for context encoder)
...
def train(X_train, y_train, model_directory_path):
    ...
    cache_path = os.path.join(LOCAL_CACHE_DIR, f"train_dataset_v30_8ch_nk{N_KERNEL}.pkl")
    ...
    path = os.path.join(model_directory_path, "model_v30.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v30_nk{N_KERNEL}.pkl") if cache_dir else None
...
if __name__ == "__main__":
    print("=" * 60)
    print("v30: 8ch Density (4 raw + 4 ANM, dual sigma) + Edge Context")
    print(f"  Node: {N_NODE_CHANNELS}ch x {GRID_SIZE}x{GRID_SIZE}")
    print(f"  Raw sigma={RAW_SIGMA}, ANM sigma={ANM_SIGMA}, ANM bw={ANM_BW}")
    print(f"  Node merge: 5 inputs (4 edge embs + 1 node_2d)")
    print(f"  Edge: v11 1D context encoder (conv1d + struct attn)")
    ...
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v30.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv30 Balanced Accuracy: {np.mean(accs):.4f}")