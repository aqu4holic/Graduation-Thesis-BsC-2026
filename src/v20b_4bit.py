"""
v20b_4bit.py — ADIA Causal Discovery
  v11 backbone + 4-bit binary node classification

Replaces the 8-class node head with 4 independent binary heads:
  head_vx(emb_vx) → P(v→X exists)
  head_vy(emb_vy) → P(v→Y exists)
  head_xv(emb_xv) → P(X→v exists)
  head_yv(emb_yv) → P(Y→v exists)

At inference, map 4 probabilities → 8 classes via pattern table.

Why this might work:
  - Binary classification is easier than 8-class (balanced, clear signal)
  - No merge bottleneck (4×64=256 → 64 compression is gone)
  - Conjunction is automatic: Mediator = P(X→v)>0.5 AND P(v→Y)>0.5
  - Each head specializes on one question
  - Edge embeddings already have global context from self-attention

The 4 binary heads are SEPARATE from the general edge_head — they
specialize for the 4 diagnostic edges only.

Training: 4× binary CE loss (from CLASS_EDGE_PATTERN[node_label])
Same backbone (conv, attention, struct bias). Same preprocessing cache.
...
# 4-bit pattern: [v→X, v→Y, X→v, Y→v]
CLASS_EDGE_PATTERN: torch.Tensor = torch.tensor([
    [1, 1, 0, 0],  # 0: Confounder
    [0, 0, 1, 1],  # 1: Collider
    [0, 1, 1, 0],  # 2: Mediator
    [1, 0, 0, 0],  # 3: Cause of X
    [0, 1, 0, 0],  # 4: Cause of Y
    [0, 0, 1, 0],  # 5: Consequence of X
    [0, 0, 0, 1],  # 6: Consequence of Y
    [0, 0, 0, 0],  # 7: Independent
...
# ============================================================
# Model Architecture — v11 backbone + 4-bit node head
# ============================================================
...
class ADIAModel(nn.Module):
    """v20b: v11 backbone + 4-bit binary node classification.

    Replaces node_merge + node_head with 4 dedicated binary heads.
    Each head: Linear(d, d//2) → GELU → Linear(d//2, 1)
    Operates on individual edge embeddings (no merge needed).
    """
    ...
    def __init__(self, d: int = None, n_edge_types: int = None, aug_noise_std: float = 0.0):
        ...
        # Edge head (identical to v11, for DAG construction)
        self.edge_head = nn.Linear(d, 2)

        # v20b: 4 dedicated binary heads — each is a small MLP
        # These replace node_merge + node_head entirely
        h = d // 2  # hidden dim = 32
        self.bit_head_vx = nn.Sequential(nn.Linear(d, h), nn.GELU(), nn.Linear(h, 1))
        self.bit_head_vy = nn.Sequential(nn.Linear(d, h), nn.GELU(), nn.Linear(h, 1))
        self.bit_head_xv = nn.Sequential(nn.Linear(d, h), nn.GELU(), nn.Linear(h, 1))
        self.bit_head_yv = nn.Sequential(nn.Linear(d, h), nn.GELU(), nn.Linear(h, 1))

        # Keep node_merge + node_head as unused for checkpoint compat (not used in forward)
        # Remove if training from scratch
    ...
    def forward(
        ...
        # v20b: 4-bit node classification
        # Returns (edge_logits, list of (K, 4) bit_logits per batch item)
        bit_logits_list: list[torch.Tensor | None] = []
        ...
        for b in range(B):
            ...
            if not other:
                bit_logits_list.append(None)
                continue

            bit_logits_batch: list[torch.Tensor] = []
            for v_name in other:
                vi = col2idx[v_name]
                xi = col2idx["X"]
                yi = col2idx["Y"]
                ...
                # 4 independent binary predictions
                bits = torch.cat([
                    self.bit_head_vx(emb_vx),  # P(v→X)
                    self.bit_head_vy(emb_vy),  # P(v→Y)
                    self.bit_head_xv(emb_xv),  # P(X→v)
                    self.bit_head_yv(emb_yv),  # P(Y→v)
                ], dim=-1)  # (4,) logits

                bit_logits_batch.append(bits)

            bit_logits_list.append(
                torch.stack(bit_logits_batch) if bit_logits_batch else None
            )

        return edge_logits, bit_logits_list


def bits_to_class(bit_logits: torch.Tensor) -> torch.Tensor:
    """Map (K, 4) bit logits to (K,) class predictions.

    Computes log-likelihood for each of 8 classes under independent
    Bernoulli model, returns argmax.
    """
    pattern = CLASS_EDGE_PATTERN.to(bit_logits.device)  # (8, 4)
    # Convert logits to probabilities
    p = torch.sigmoid(bit_logits).clamp(min=1e-6, max=1 - 1e-6)  # (K, 4)
    log_p = torch.log(p)      # (K, 4)
    log_1mp = torch.log(1 - p)  # (K, 4)
    # Score each class: sum of log-likelihoods
    # (K, 1, 4) * (1, 8, 4) → (K, 8, 4) → sum → (K, 8)
    scores = (log_p.unsqueeze(1) * pattern.unsqueeze(0) +
              log_1mp.unsqueeze(1) * (1 - pattern).unsqueeze(0)).sum(dim=-1)
    return torch.argmax(scores, dim=-1)  # (K,)
...
class ADIAModelWrapper(pl.LightningModule):
    def __init__(
        ...
        # v20b: BCE for 4-bit classification
        self.bit_criterion = nn.BCEWithLogitsLoss(reduction='none')

        # Register pattern as buffer
        self.register_buffer('class_edge_pattern', CLASS_EDGE_PATTERN)

        # Precompute per-bit class weights for balanced training
        # Weight each bit's loss by inverse frequency of 1s vs 0s across classes
        # (weighted by class frequency)
        if node_class_weights is not None:
            # node_class_weights: (8,) — inverse freq weights per class
            # Compute effective weight for bit=1 vs bit=0 for each of 4 bits
            w = node_class_weights  # (8,)
            pattern = CLASS_EDGE_PATTERN  # (8, 4)
            # Weighted sum of classes where bit=1
            w1 = (w.unsqueeze(1) * pattern).sum(0)  # (4,)
            # Weighted sum of classes where bit=0
            w0 = (w.unsqueeze(1) * (1 - pattern)).sum(0)  # (4,)
            # Normalize
            self.register_buffer('bit_pos_weight', w1 / (w0 + 1e-6))
        else:
            self.register_buffer('bit_pos_weight', torch.ones(4))
    ...
    def _compute_loss(self, batch: dict, split: str) -> torch.Tensor:
        ...
        # v20b: 4-bit binary loss (replaces 8-class node CE)
        if "node_labels" in batch:
            all_bit_logits: list[torch.Tensor] = []
            all_bit_targets: list[torch.Tensor] = []
            nl = batch["node_labels"].to(self.device)

            for b in range(B):
                if bit_logits_list[b] is not None:
                    K = bit_logits_list[b].shape[0]
                    labels_b = nl[b, :K]
                    valid = labels_b >= 0

                    if valid.any():
                        valid_logits = bit_logits_list[b][valid]  # (V, 4)
                        valid_labels = labels_b[valid]  # (V,)
                        # Look up 4-bit targets from pattern table
                        targets = self.class_edge_pattern[valid_labels]  # (V, 4)
                        all_bit_logits.append(valid_logits)
                        all_bit_targets.append(targets)

            if all_bit_logits:
                cat_logits = torch.cat(all_bit_logits, dim=0)  # (N, 4)
                cat_targets = torch.cat(all_bit_targets, dim=0)  # (N, 4)

                # Weighted BCE: pos_weight per bit for class balance
                raw_loss = self.bit_criterion(cat_logits, cat_targets)  # (N, 4)
                # Apply pos_weight: scale loss for positive examples
                weight = torch.where(
                    cat_targets > 0.5,
                    self.bit_pos_weight.unsqueeze(0),
                    torch.ones_like(raw_loss),
                )
                bit_loss = (raw_loss * weight).mean()

                total_loss = total_loss + bit_loss
                n_terms += 1
                self.log(f"{split}/bit_loss", bit_loss, prog_bar=True, sync_dist=True)

                # Log per-bit accuracy for monitoring
                with torch.no_grad():
                    preds = (cat_logits > 0).float()
                    bit_acc = (preds == cat_targets).float().mean()
                    self.log(f"{split}/bit_acc", bit_acc, prog_bar=True, sync_dist=True)
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    ...
    for batch in tqdm(loader, desc="inferring"):
        edge_logits, bit_logits_list = model(
        ...
        for b in range(B_cur):
            ...
            if bit_logits_list[b] is not None:
                ...
                # v20b: map 4-bit logits to class predictions
                node_preds = bits_to_class(bit_logits_list[b])
...
if __name__ == "__main__":
    print("=" * 60)
    print("v20b: 4-Bit Binary Node Classification")
    print(f"  4 dedicated binary heads replace 8-class node_merge+head")
    print(f"  Pattern matching: 4 bit probs → 8 class scores")
