"""
v26c_pure_node2d.py — ADIA Causal Discovery
  Pure node-centric 8-channel 2D density — NO edge pipeline at all

Same 8-channel representation as v26:
...
NO edge pipeline — no conv1d, no self-attention, no edge_head, no edge_merge.
Each node classified purely from its 8-channel density image.
Tests whether the 8ch 2D node representation is self-sufficient
(v24b tried this with 4ch and failed at 53.95%, but had bad preprocessing).

If this works well, it proves the 2D density signal is strong enough alone.
If it fails, it proves self-attention context is needed (and v26 with fusion is the way).

Usage:
    python v26c_pure_node2d.py
"""
...
# ============================================================
# Model Architecture — pure conv2d, no edge pipeline
# ============================================================
...
class ADIAModel(nn.Module):
    """v26c: PURE node 2D. No edge pipeline at all.
    8ch density image → conv2d → node_head (8-class).
    """
...
def train(X_train, y_train, model_directory_path):
    ...
    cache_path = os.path.join(LOCAL_CACHE_DIR, f"train_dataset_v26c_pure8ch_nk{N_KERNEL}.pkl")
    ...
    trainer.fit(wrapper, loader, ckpt_path="/data/anhld48/phatnt/Graduation-Thesis-BsC-2026/src/causal-discovery-thesis/9nh4u8uf/checkpoints/epoch=29-step=22050.ckpt")

    path = os.path.join(model_directory_path, "model_v26c.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v26c_nk{N_KERNEL}.pkl") if cache_dir else None
...
if __name__ == "__main__":
    print("=" * 60)
    print("v26c: PURE Node 2D (8ch, no edge pipeline)")
    ...
    print(f"  NO edge pipeline, NO self-attention, NO edge_head")
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ADIAModel(d=D_MODEL)
    model.load_state_dict(torch.load("resources/model_v26c.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv26c Balanced Accuracy: {np.mean(accs):.4f}")