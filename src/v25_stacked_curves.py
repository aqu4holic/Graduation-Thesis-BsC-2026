"""
v25_stacked_curves.py — ADIA Causal Discovery
  Stacked sorted curves (6×N_OBS) + edge attention with fused node embeddings

Key insight: Instead of rendering scatter plots as 32x32 density histograms (lossy),
stack the sorted observation curves as rows of a tall 2D image. For each node v,
build a (6, N_OBS) "image":
  Row 0: v sorted by X
  Row 1: X sorted by v
  Row 2: v sorted by Y
  Row 3: Y sorted by v
  Row 4: X sorted by Y (context, same for all v in the sample)
  Row 5: Y sorted by X (context, same for all v in the sample)

Conv2d with full vertical span processes all 6 rows simultaneously, then conv1d
blocks capture within-curve spatial structure. The vertical kernel crosses rows
to learn directional patterns (e.g., "row 0 has trend but row 1 doesn't" → causal).

Critical fix vs v24: FUSE node 2D embedding with edge embeddings via node_merge
with n_inputs=5 (4 edge directions + 1 node_2d).

Usage:
    python v25_stacked_curves.py
"""
...
N_OBS: int = 1000
N_KERNEL: int = 1000
N_NODE_CHANNELS: int = 6  # 6 sorted curve rows
N_CLASSES: int = 8
N_EDGE_TYPES: int = 7
D_MODEL: int = 64
N_HEADS: int = 4
...
# ============================================================
# 2D Preprocessing — stacked sorted curves (6 x N_OBS)
# ============================================================
def build_node_images(df):
    """Build 6-row stacked sorted curve images for each non-X/Y node.

    For each node v, returns (6, N_OBS):
      Row 0: v sorted by X
      Row 1: X sorted by v
      Row 2: v sorted by Y
      Row 3: Y sorted by v
      Row 4: X sorted by Y (context, same for all v)
      Row 5: Y sorted by X (context, same for all v)

    Raw sorted values, standardized per-sample (no rank transform, no histogram).
    """
    cols = list(df.columns)
    data = df.values.astype(np.float32)
    ...
    # Precompute context rows (same for all v in this sample)
    sort_by_y = np.argsort(y_data)
    sort_by_x = np.argsort(x_data)
    row4 = x_std[sort_by_y]  # X sorted by Y
    row5 = y_std[sort_by_x]  # Y sorted by X
    ...
    for v_name in other_nodes:
        ...
        sort_by_x_idx = sort_by_x  # already computed
        sort_by_v = np.argsort(v_data)
        sort_by_y_idx = sort_by_y  # already computed

        row0 = v_std[sort_by_x_idx]   # v sorted by X
        row1 = x_std[sort_by_v]       # X sorted by v
        row2 = v_std[sort_by_y_idx]   # v sorted by Y
        row3 = y_std[sort_by_v]       # Y sorted by v

        node_images[v_name] = np.stack([row0, row1, row2, row3, row4, row5], axis=0)  # (6, N_OBS)
...
def _build_single(args):
    ...
    node_images = build_node_images(df)
    ...
    result = {
        ...
        "cols": cols,
        "p": p,
        "node_images": node_images,  # dict: node_name → (6, N_OBS)
...
def collate_fn(batch):
    ...
    # Node images: (B, max_K, 6, N_OBS)
    node_imgs = torch.zeros(B, max_K, N_NODE_CHANNELS, N_OBS)
...
class NodeFeatureExtractor2D(nn.Module):
    """Stacked curves backbone for 6-row node images with hierarchical downsampling.

    Input: (B_nodes, 6, N_OBS)
    Output: (B_nodes, d)

    Stem: Conv2d crosses all 6 rows → (d, 1, N) → squeeze to (d, N).
    Then progressive downsampling via strided conv1d:
    N=1000 → 500 → 125 → 32 → pool → d
    Ensures receptive field covers the full sequence.
    """
    def __init__(self, d=64, n_blocks=5, n_channels=6):
        super().__init__()
        # Full vertical span (6), moderate horizontal (7) with padding to preserve width
        self.stem = nn.Conv2d(1, d, kernel_size=(n_channels, 7), padding=(0, 3))
        # Progressive downsampling: 1000 → 500 → 125 → 32
        self.down = nn.Sequential(
            ConvBlock1D(d),
            nn.Conv1d(d, d, 3, stride=2, padding=1),  # N/2
            nn.GELU(),
            ConvBlock1D(d),
            nn.Conv1d(d, d, 3, stride=4, padding=1),  # N/8
            nn.GELU(),
            ConvBlock1D(d),
            nn.Conv1d(d, d, 3, stride=4, padding=1),  # N/32
            nn.GroupNorm(8, d),
            nn.GELU(),
        )
        self.pool = nn.AdaptiveAvgPool1d(1)
    ...
    def forward(self, x):
        # x: (B, 6, N_OBS)
        # Treat as single-channel 2D image: (B, 1, 6, N_OBS)
        x = x.unsqueeze(1)
        x = self.stem(x)            # (B, d, 1, N_OBS)
        x = x.squeeze(2)            # (B, d, N_OBS)
        x = self.down(x)            # (B, d, ~32)
...
class ADIAModel(nn.Module):
    """v25: Stacked sorted curves + edge attention with fused node embeddings.

    Edge pipeline: identical to v11 (conv1d -> attention -> edge_head)
    Node pipeline: conv2d on 6-row stacked curves -> fuse with 4 edge embeddings -> node_head
    """
    ...
    def __init__(self, d=None, n_edge_types=None, aug_noise_std=0.0):
        ...
        # Node pipeline (v25: stacked curves + fused with edge embeddings)
        self.node_extractor = NodeFeatureExtractor2D(d, n_blocks=5, n_channels=N_NODE_CHANNELS)
        self.node_merge = MergeOperator(n_inputs=5, d=d)  # 4 edge dirs + 1 node_2d
    ...
    def forward(self, edge_data, edge_types, edge_mask, cols_list,
        ...
        # Node pipeline — process stacked curve images and fuse with edge embeddings
        node_logits_list = []
        if node_imgs is not None:
            B_n, K, C_n, N_obs = node_imgs.shape
            # Process all node images through conv2d+conv1d backbone
            flat_imgs = node_imgs.view(B_n * K, C_n, N_obs)
            node_2d_embs = self.node_extractor(flat_imgs)  # (B*K, d)
            node_2d_embs = node_2d_embs.view(B_n, K, self.d)

            # Build edge index lookup: for a given (u_idx, v_idx), find the edge position
            # Edge ordering: for u in range(p): for v in range(p): if u != v: append (u,v)
            for b in range(B):
                cols = cols_list[b]
                p = len(cols)
                col2idx = {c: i for i, c in enumerate(cols)}
                ...
                if K_b > 0:
                    batch_node_logits = []
                    for k in range(K_b):
                        v_name = other_nodes[k]
                        vi = col2idx[v_name]
                        embs = [
                            edge_emb[b, eidx_map[(vi, xi_idx)]],   # v->X
                            edge_emb[b, eidx_map[(vi, yi_idx)]],   # v->Y
                            edge_emb[b, eidx_map[(xi_idx, vi)]],   # X->v
                            edge_emb[b, eidx_map[(yi_idx, vi)]],   # Y->v
                            node_2d_embs[b, k],                     # node 2D context
                        ]
                        node_emb = self.node_merge(embs)
                        batch_node_logits.append(self.node_head(node_emb))
                    node_logits_list.append(torch.stack(batch_node_logits, dim=0))
                else:
...
def train(X_train, y_train, model_directory_path):
    ...
    cache_path = os.path.join(LOCAL_CACHE_DIR, f"train_dataset_v25_stacked_nk{N_KERNEL}.pkl")
    ...
    trainer.fit(wrapper, loader, ckpt_path="/data/anhld48/phatnt/Graduation-Thesis-BsC-2026/src/causal-discovery-thesis/tft9lova/checkpoints/epoch=20-step=30849.ckpt")

    path = os.path.join(model_directory_path, "model_v25.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v25_nk{N_KERNEL}.pkl") if cache_dir else None
...
if __name__ == "__main__":
    print("=" * 60)
    print("v25: Stacked Sorted Curves (6xN) + Edge Attention Fusion")
    print(f"  Node: {N_NODE_CHANNELS}ch x {N_OBS} stacked sorted curves")
    print(f"  Edge: v11 1D pipeline (conv1d + struct attn)")
    print(f"  Fusion: node_merge(4 edge embs + node_2d_emb)")
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v25.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv25 Balanced Accuracy: {np.mean(accs):.4f}")