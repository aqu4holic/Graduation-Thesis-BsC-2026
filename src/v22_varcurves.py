"""
v22_varcurves.py — ADIA Causal Discovery
  v11 + conditional variance + IGCI curves (new preprocessing)

New channels (all genuine 1D curves with spatial structure):

  ch9:  Var(j|i) — conditional variance curve sorted by i
        Computed as kernel regression of squared residuals.
        Captures heteroscedasticity: if Y = f(X) + g(X)*eps,
        Var(Y|X) varies with X. Direction-dependent signal.

  ch10: Local slope |dE[j|i]/di| — mechanism complexity curve
        Numerical derivative of the conditional expectation.
        IGCI principle: in causal direction, slope complexity
        is independent of input density.

  ch11: Local density p(i) — cause distribution curve
        KDE estimate of source variable density sorted by i.
        IGCI principle: combined with slope, reveals causal direction.

N_CHANNELS: 8 -> 11

All three are CURVES sorted by source — conv1d can learn spatial
patterns. NOT broadcast scalars (which we've proven fail in v3/v14/v15).

Each captures fundamentally different information:
  - Existing channels: conditional MEAN E[j|i]
  - Var(j|i): conditional VARIANCE (2nd moment)
  - Slope: MECHANISM complexity
  - Density: CAUSE distribution

Same architecture as v11. REQUIRES NEW CACHE.

Usage:
    python v22_varcurves.py
"""
...
N_CHANNELS: int = 2 + len(BANDWIDTHS) + len(BANDWIDTHS) + 3  # 11
...
# ============================================================
# Data Preprocessing — v11 + variance + IGCI curves
# ============================================================
...
def compute_local_slope(
    cond_expect: np.ndarray, window: int = 21,
) -> np.ndarray:
    """Compute |dE[j|i]/di| — local slope of conditional expectation.

    Vectorized finite differences with adaptive window at boundaries.

    Returns (N,) float32 — absolute local slope at each sorted position.
    """
    N: int = len(cond_expect)
    half_w: int = window // 2

    idx: np.ndarray = np.arange(N)
    lo: np.ndarray = np.clip(idx - half_w, 0, N - 1)
    hi: np.ndarray = np.clip(idx + half_w, 0, N - 1)
    dx: np.ndarray = (hi - lo).astype(np.float32)
    dx = np.maximum(dx, 1.0)
    dy: np.ndarray = cond_expect[hi] - cond_expect[lo]

    return np.abs(dy / dx).astype(np.float32)


def compute_local_density(
    source_sorted: np.ndarray, bandwidth: float = 0.5,
) -> np.ndarray:
    """Compute p(source) via KDE, evaluated at sorted source positions.

    Returns (N,) float32 — density estimate at each sorted position.
    """
    N: int = len(source_sorted)

    # Subsample for efficiency (KDE on full N×N is expensive)
    n_kde: int = min(500, N)
    step: int = max(1, N // n_kde)
    kde_points: np.ndarray = source_sorted[::step]

    # KDE: p(x) = (1/n*h) * sum(K((x - x_i)/h))
    diff: np.ndarray = source_sorted[:, None] - kde_points[None, :]  # (N, n_kde)
    K: np.ndarray = np.exp(-diff ** 2 / (2 * bandwidth ** 2))
    density: np.ndarray = K.mean(axis=1) / (bandwidth * np.sqrt(2 * np.pi))

    # Avoid log(0) issues — clamp to small positive
...
def build_edge_tensor(
    ...
    # Standard multivariate kernel regression (same as v11)
    ...
    for i, u_name in enumerate(cols):
        ...
        # v22: precompute local density for source i (same for all targets)
        density_curve: np.ndarray = compute_local_density(u_sorted, bandwidth=0.5)

        # v22 fix: compute NW kernel weights ONCE per source i, reuse for all targets j
        source: np.ndarray = data[:, i]
        source_sub: np.ndarray = source[sub_idx]
        diff_sq: np.ndarray = (source[:, None] - source_sub[None, :]) ** 2  # (N, n_sub)
        W_nw: np.ndarray = np.exp(-diff_sq / (2 * 0.5 ** 2))
        W_nw_sum: np.ndarray = W_nw.sum(axis=1, keepdims=True) + 1e-10
        W_nw_norm: np.ndarray = W_nw / W_nw_sum  # (N, n_sub)
        ...
        for j, v_name in enumerate(cols):
            ...
            # v22 ch9: Conditional variance Var(j|i) — reuse W_nw_norm
            target_sub: np.ndarray = data[sub_idx, j]
            target_hat: np.ndarray = W_nw_norm @ target_sub  # (N,)
            resid_sq_sub: np.ndarray = (target_sub - target_hat[sub_idx]) ** 2
            cond_var: np.ndarray = (W_nw_norm @ resid_sq_sub)[sort_idx].astype(np.float32)

            # v22 ch10: Local slope |dE[j|i]/di| — vectorized
            slope_curve: np.ndarray = compute_local_slope(v_sorted_by_u)

            # v22 ch11: Local density p(i) — already computed above
            channels: list[np.ndarray] = (
                [u_sorted, v_sorted_by_u]
                + kernel_channels
                + anm_channels
                + [cond_var, slope_curve, density_curve]
            )
...
# ============================================================
# Model Architecture (identical to v11, just N_CHANNELS=11)
# ============================================================
...
class ADIAModel(nn.Module):
    """v22: v11 architecture with N_CHANNELS=11."""
...
def train(
    ...
    cache_path: str = os.path.join(
        LOCAL_CACHE_DIR, f"train_dataset_v22_varcurves_nk{N_KERNEL}.pkl"
    )
    ...
    path: str = os.path.join(model_directory_path, "model_v22.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v22_nk{N_KERNEL}.pkl") if cache_dir else None
...
def infer(X_test, model_directory_path, id_column_name, prediction_column_name):
    path = os.path.join(model_directory_path, "model_v22.pt")
...
if __name__ == "__main__":
    print("=" * 60)
    print("v22: Conditional Variance + IGCI Curves")
    print(f"  N_CHANNELS: {N_CHANNELS} (was 8)")
    print(f"  New: Var(j|i), local slope, local density")
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v22.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
