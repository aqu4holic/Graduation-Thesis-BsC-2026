"""
v25a_scatter_density.py — ADIA Causal Discovery
  Scatter density 4-channel 2D images + edge attention with FUSED node+edge embeddings

Key changes from v24 (which failed at 53.54%):
  1. NO rank transform — raw values, min-max normalized to [0,1] per variable
  2. NO row normalization — keep JOINT density (not conditional)
  3. Larger Gaussian sigma = 2.0 (v24 used 1.0)
  4. ch3 is (X,Y) context — gives the model the treatment-outcome relationship
  5. FUSE node_2d_emb with 4 edge embeddings via node_merge (n_inputs=5)
     v24 classified nodes independently from conv2d with NO edge context

For each node v, create a 4-channel 32x32 image:
  ch0: scatter density of (v, X) — joint density
  ch1: scatter density of (v, Y) — joint density
  ch2: scatter density of (X, v) = ch0 transposed
  ch3: scatter density of (Y, v) — ch1 transposed

Architecture: edge-level 1D pipeline (same as v11) for edge_head,
PLUS node-level 2D pipeline FUSED with edge embeddings for node classification.

Usage:
    python v25a_scatter_density.py
"""
...
N_KERNEL: int = 1000
GRID_SIZE: int = 32
N_NODE_CHANNELS: int = 4  # 4 scatter density channels
SCATTER_SIGMA: float = 5.0  # Gaussian smoothing sigmaint x;
N_CLASSES: int = 8
N_EDGE_TYPES: int = 7
D_MODEL: int = 64
N_HEADS: int = 4
...
# ============================================================
# 2D Preprocessing — scatter density images (v25a: NO rank, NO row-norm)
# ============================================================
def build_scatter_density(source, target, grid_size=32, sigma=2.0):
    """Render scatter plot as 32x32 density image.
    Uses min-max normalization (NOT rank transform).
    Returns joint density (NOT conditional/row-normalized)."""
    ...
    # Gaussian smoothing to magnify signal
    hist_smooth = gaussian_filter(hist.astype(np.float32), sigma=sigma)
    # Normalize to sum=1 (density, NOT row-normalized)
...
def build_node_images(df):
    """Build 4-channel scatter density images for each non-X/Y node.

    For each node v, returns (4, GRID_SIZE, GRID_SIZE):
      ch0: scatter density of (v, X) — joint density
      ch1: scatter density of (v, Y) — joint density
      ch2: scatter density of (X, v) = ch0 transposed
      ch3: scatter density of (X, Y) — context, same for all v
    """
    cols = list(df.columns)
    data = df.values.astype(np.float32)
    ...
    # Precompute (X, Y) context channel — same for all nodes in this sample
    x_data = data[:, xi]
    y_data = data[:, yi]
    # ch3_xy = build_scatter_density(x_data, y_data, GRID_SIZE, sigma=SCATTER_SIGMA)
    ...
    for v_name in other_nodes:
        ...
        ch2 = ch0.T.copy()  # (X, v) = transpose of (v, X)
        ch3 = ch1.T.copy()

        node_images[v_name] = np.stack([ch0, ch1, ch2, ch3], axis=0)  # (4, H, W)
...
def _build_single(args):
    ...
    result = {
        ...
        "cols": cols,
        "p": p,
        "node_images": node_images,  # dict: node_name -> (4, H, W)
...
class NodeFeatureExtractor2D(nn.Module):
    """Hierarchical conv2d with progressive downsampling for scatter density images.

    Input: (B_nodes, 4, 32, 32)
    Output: (B_nodes, d)

    Progressive downsampling ensures the receptive field covers the full image,
    so global patterns (diagonal causal curves) are visible before pooling.
    32×32 → 16×16 → 8×8 → 4×4 → pool → d
    """
    def __init__(self, d=64, n_blocks=5, n_channels=4):
        super().__init__()
...
class ADIAModel(nn.Module):
    """v25a: Edge-level 1D pipeline (edge_head) + node-level 2D pipeline FUSED with edges (node_head).

    Edge pipeline: identical to v11 (conv1d -> attention -> edge_head)
    Node pipeline: conv2d on 4-channel scatter density images -> merge with 4 edge embeddings -> node_head (8-class)

    Critical fix from v24: v24 classified nodes independently from conv2d output
    with NO context from edges. v25a merges node_2d_emb with 4 edge embeddings.
    """
    ...
    def __init__(self, d=None, n_edge_types=None, aug_noise_std=0.0):
        ...
        # Node pipeline (v25a: fused with edge embeddings)
    ...
    def forward(self, edge_data, edge_types, edge_mask, cols_list,
        ...
        # Node pipeline — process 2D images and FUSE with edge embeddings
        node_logits_list = []
        if node_imgs is not None:
            B_n, K, C_n, H, W = node_imgs.shape
            # Flatten to (B*K, C, H, W), process through conv2d
            flat_imgs = node_imgs.view(B_n * K, C_n, H, W)
            flat_emb = self.node_extractor(flat_imgs)  # (B*K, d)
            node_2d_embs = flat_emb.view(B_n, K, self.d)  # (B, K, d)

            # Helper to compute edge index in the flattened edge list
            # Edge ordering: for each u in range(p), for each v in range(p), if u!=v
            def _eidx(p, u, v):
                """Get edge index for directed edge u->v in a graph with p nodes."""
                # Edges are ordered: for u in 0..p-1, for v in 0..p-1, v!=u
                idx = 0
                for uu in range(p):
                    for vv in range(p):
                        if uu == vv:
            ...
            for b in range(B):
                ...
                if K_b > 0:
                    ...
                    for k, v_name in enumerate(other_nodes[:K_b]):
                        vi = col2idx[v_name]
                        # Get 4 edge embeddings for this node
                        e_vx = _eidx(p, vi, xi)  # v -> X
                        e_vy = _eidx(p, vi, yi)  # v -> Y
                        e_xv = _eidx(p, xi, vi)  # X -> v
                        e_yv = _eidx(p, yi, vi)  # Y -> v
                        ...
                        embs = [
                            ...
                            edge_emb[b, e_xv],     # X -> v
                            edge_emb[b, e_yv],     # Y -> v
                            node_2d_embs[b, k],    # scatter density context
                        ]
                        merged = self.node_merge(embs)  # (d,)
                        per_node_logits.append(self.node_head(merged))  # (N_CLASSES,)

                    node_logits_list.append(torch.stack(per_node_logits, dim=0))  # (K_b, N_CLASSES)
                else:
...
def train(X_train, y_train, model_directory_path):
    ...
    cache_path = os.path.join(LOCAL_CACHE_DIR, f"train_dataset_v25a_scatter_nk{N_KERNEL}.pkl")
    ...
    path = os.path.join(model_directory_path, "model_v25a.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v25a_nk{N_KERNEL}.pkl") if cache_dir else None
...
if __name__ == "__main__":
    print("=" * 60)
    print("v25a: Scatter Density 2D + Edge-Fused Node Classification")
    print(f"  Node: 4ch x {GRID_SIZE}x{GRID_SIZE} scatter densities (sigma={SCATTER_SIGMA})")
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v25a.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv25a Balanced Accuracy: {np.mean(accs):.4f}")