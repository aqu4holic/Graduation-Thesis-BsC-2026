"""
v24_node2d.py — ADIA Causal Discovery
  Node-centric 4-channel 2D conditional density + edge attention

Key insight: the 8-class problem is fundamentally about TRIPLETS (v, X, Y),
not pairs. Processing pairs independently then merging loses conjunction info.

For each node v, create a 4-channel 32x32 image:
  ch0: P(rank_X | rank_v) — conditional density, "does v cause X?"
  ch1: P(rank_Y | rank_v) — conditional density, "does v cause Y?"
  ch2: P(rank_v | rank_X) — conditional density, "does X cause v?"
  ch3: P(rank_v | rank_Y) — conditional density, "does Y cause v?"

Conv2d processes all 4 channels simultaneously → can learn conjunctions:
  ch0 signal + ch1 signal → Confounder (v→X AND v→Y)
  ch2 signal + ch1 signal → Mediator (X→v AND v→Y)

Architecture: edge-level 1D pipeline (same as v11) for edge_head,
PLUS node-level 2D pipeline for node classification. The node 2D
branch replaces node_merge + node_head entirely.

Usage:
    python v24_node2d.py
"""
...
N_KERNEL: int = 1000
GRID_SIZE: int = 32
N_NODE_CHANNELS: int = 4  # 4 conditional densities per node
N_CLASSES: int = 8
N_EDGE_TYPES: int = 7
D_MODEL: int = 64
N_HEADS: int = 4
...
# ============================================================
# 2D Preprocessing — node-centric conditional density images
# ============================================================
def rank_transform(x):
    return (np.argsort(np.argsort(x)).astype(np.float32) + 0.5) / len(x)


def build_conditional_density_image(source, target, grid_size=32, sigma=1.0):
    """Build P(rank_target | rank_source) as a 2D density image.

    x-axis = rank_source bins, y-axis = rank_target bins.
    Each column is the conditional distribution of target given source in that bin.
    """
    N = len(source)
    rank_s = rank_transform(source)
    rank_t = rank_transform(target)

    # 2D histogram
    hist, _, _ = np.histogram2d(
        rank_s, rank_t, bins=grid_size, range=[[0, 1], [0, 1]]
    )
    # Gaussian smoothing
    hist_smooth = gaussian_filter(hist.astype(np.float32), sigma=sigma)
    # Row-normalize → P(target | source)
    row_sums = hist_smooth.sum(axis=1, keepdims=True)
    row_sums = np.maximum(row_sums, 1e-10)
    cond = hist_smooth / row_sums
    return cond.astype(np.float32)


def build_node_images(df):
    """Build 4-channel conditional density images for each non-X/Y node.

    For each node v, returns (4, GRID_SIZE, GRID_SIZE):
      ch0: P(rank_X | rank_v)  — v→X direction
      ch1: P(rank_Y | rank_v)  — v→Y direction
      ch2: P(rank_v | rank_X)  — X→v direction
      ch3: P(rank_v | rank_Y)  — Y→v direction
    """
    cols = list(df.columns)
    data = df.values.astype(np.float32)
    ...
    for v_name in other_nodes:
        ...
        ch0 = build_conditional_density_image(v_data, x_data, GRID_SIZE, sigma=1.0)
        ch1 = build_conditional_density_image(v_data, y_data, GRID_SIZE, sigma=1.0)
        ch2 = build_conditional_density_image(x_data, v_data, GRID_SIZE, sigma=1.0)
        ch3 = build_conditional_density_image(y_data, v_data, GRID_SIZE, sigma=1.0)
...
class NodeFeatureExtractor2D(nn.Module):
    """Conv2d backbone for 4-channel node density images.
...
class ADIAModel(nn.Module):
    """v24: Edge-level 1D pipeline (edge_head) + node-level 2D pipeline (node_head).

    Edge pipeline: identical to v11 (conv1d → attention → edge_head)
    Node pipeline: conv2d on 4-channel density images → node_head (8-class)
    """
    ...
    def forward(self, edge_data, edge_types, edge_mask, cols_list,
        ...
        # Node pipeline — process 2D images
        node_logits_list = []
        if node_imgs is not None:
            B_n, K, C_n, H, W = node_imgs.shape
            # Flatten to (B*K, C, H, W), process, reshape
            flat_imgs = node_imgs.view(B_n * K, C_n, H, W)
            flat_emb = self.node_extractor(flat_imgs)  # (B*K, d)
            flat_logits = self.node_head(flat_emb)       # (B*K, 8)
            all_logits = flat_logits.view(B_n, K, N_CLASSES)

            for b in range(B):
                K_b = int(node_img_mask[b].sum().item()) if node_img_mask is not None else K
                if K_b > 0:
...
def train(X_train, y_train, model_directory_path):
    ...
    cache_path = os.path.join(LOCAL_CACHE_DIR, f"train_dataset_v24_node2d_nk{N_KERNEL}.pkl")
    ...
    path = os.path.join(model_directory_path, "model_v24.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v24_nk{N_KERNEL}.pkl") if cache_dir else None
    if cache_path and os.path.exists(cache_path):
        ...
        # print(f"Building edge+node tensors ({len(dfs)} graphs, {n_workers} workers)...")
        # ctx = mp.get_context('fork')
        # with ProcessPoolExecutor(max_workers=n_workers, mp_context=ctx) as pool:
...
if __name__ == "__main__":
    print("=" * 60)
    print("v24: Node-Centric 2D Density + Edge Attention")
    print(f"  Node: 4ch x {GRID_SIZE}x{GRID_SIZE} conditional densities")
    ...
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v24.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv24 Balanced Accuracy: {np.mean(accs):.4f}")