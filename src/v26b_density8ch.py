"""
v26b_density8ch.py — ADIA Causal Discovery
...
SCATTER_SIGMA: float = 5.0  # Gaussian smoothing sigma for 2D histograms
...
class ADIAModel(nn.Module):
    """v26b: Edge 1D pipeline + 8ch node 2D pipeline FUSED with edge embeddings.
    ...
    def __init__(self, d=None, n_edge_types=None, aug_noise_std=0.0):
        ...
        # Node pipeline (v26b: 8ch density fused with edges)
...
def train(X_train, y_train, model_directory_path):
    ...
    cache_path = os.path.join(LOCAL_CACHE_DIR, f"train_dataset_v26b_8ch_nk{N_KERNEL}.pkl")
    ...
    path = os.path.join(model_directory_path, "model_v26b.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v26b_nk{N_KERNEL}.pkl") if cache_dir else None
...
if __name__ == "__main__":
    print("=" * 60)
    print("v26: 8-Channel Density (4 raw + 4 ANM) + Edge-Fused Node")
    print(f"  Node: {N_NODE_CHANNELS}ch x {GRID_SIZE}x{GRID_SIZE} (sigma={SCATTER_SIGMA}, anm_bw={ANM_BW})")
    ...
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v26b.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv26 Balanced Accuracy: {np.mean(accs):.4f}")
