"""
v23_2d.py — ADIA Causal Discovery
  2D copula density representation + conv2d backbone

Fundamental change: replaces 1D sorted curves with 2D conditional
density images. Each edge (i->j) is represented as a 3-channel
32x32 image:

  ch0: Copula density — KDE-smoothed 2D histogram of (rank_i, rank_j)
       Captures the full joint dependence structure, marginal-free.

  ch1: Conditional density — row-normalized copula P(rank_j | rank_i)
       Directional signal: if i->j is causal, conditional P(j|i)
       is "clean" (unimodal). If j->i, conditional is "messy".

  ch2: ANM copula — scatter of (rank_i, rank_resid_j) at bw=0.5
       2D version of the ANM independence test. If i->j is causal
       and noise is additive, this should look uniform.

Why 2D might beat 1D:
  - 1D captures E[j|i] (conditional mean only)
  - 2D captures FULL conditional distribution (spread, multi-modality)
  - Conv2d learns patterns in the density shape, not just the mean curve
  - Heteroscedasticity is directly visible in the 2D image width

Memory: 3x32x32 = 3,072 per edge vs 8x1000 = 8,000 (current). LESS.

Architecture: Conv2d backbone replaces Conv1d. Everything else
(edge merge, structural attention, node head) identical to v11.
...
# ============================================================
GRID_SIZE: int = 32
N_2D_CHANNELS: int = 3  # copula, conditional, anm_copula
N_KERNEL: int = 1000
N_CLASSES: int = 8
N_EDGE_TYPES: int = 7
D_MODEL: int = 64
...
BATCH_SIZE: int = 16
LR: float = 1e-3
AUG_NOISE_STD: float = 0.0  # no noise aug for 2D (density images are already smooth)
...
# ============================================================
# Data Preprocessing — 2D copula density images
# ============================================================
...
def rank_transform(x: np.ndarray) -> np.ndarray:
    ...
    order: np.ndarray = np.argsort(np.argsort(x))
    return (order + 0.5) / len(x)


def build_copula_density(
    rank_u: np.ndarray, rank_v: np.ndarray,
    grid_size: int = 32, sigma: float = 1.0,
) -> np.ndarray:
    """Build KDE-smoothed 2D copula density on [0,1]x[0,1] grid.
    ...
    hist, _, _ = np.histogram2d(
        rank_u, rank_v,
        bins=grid_size, range=[[0, 1], [0, 1]],
    )
    # Gaussian smoothing for KDE-like effect
    hist_smooth: np.ndarray = gaussian_filter(hist.astype(np.float32), sigma=sigma)
    # Normalize to density
    total: float = hist_smooth.sum()
    if total > 0:
        hist_smooth /= total
    return hist_smooth


def build_conditional_density(copula: np.ndarray) -> np.ndarray:
    """Row-normalize copula density to get P(j|i).

    Each row k represents P(target | source in bin k).
    Returns (grid_size, grid_size) float32 array.
    """
    row_sums: np.ndarray = copula.sum(axis=1, keepdims=True)
    row_sums = np.maximum(row_sums, 1e-10)
    return (copula / row_sums).astype(np.float32)


def compute_anm_residuals_pairwise(
    source: np.ndarray, target: np.ndarray,
    sub_idx: np.ndarray, bandwidth: float = 0.5,
) -> np.ndarray:
    """Compute target - E[target|source] via 1D kernel regression."""
    N: int = len(source)
    n_sub: int = len(sub_idx)
    source_sub: np.ndarray = source[sub_idx]
    target_sub: np.ndarray = target[sub_idx]

    diff: np.ndarray = source[:, None] - source_sub[None, :]
    W: np.ndarray = np.exp(-diff ** 2 / (2 * bandwidth ** 2))
    W_sum: np.ndarray = W.sum(axis=1, keepdims=True) + 1e-10
    W_norm: np.ndarray = W / W_sum
    target_hat: np.ndarray = W_norm @ target_sub

    return (target - target_hat).astype(np.float32)


def build_edge_tensor_2d(
    df: pd.DataFrame,
) -> tuple[np.ndarray, np.ndarray]:
    """Build 2D copula density images for all directed edges.

    Returns:
        edge_data: (E, 3, GRID_SIZE, GRID_SIZE) float32
        edge_types: (E,) int64
    """
    ...
    # Precompute rank transforms for all variables
    ranks: np.ndarray = np.zeros_like(data)
    for k in range(p):
        ranks[:, k] = rank_transform(data[:, k])

    edges: list[np.ndarray] = []
    edge_types: list[int] = []

    for i, u_name in enumerate(cols):
        rank_u: np.ndarray = ranks[:, i]

        for j, v_name in enumerate(cols):
            if i == j:
                continue
            rank_v: np.ndarray = ranks[:, j]

            # ch0: Copula density P(rank_i, rank_j)
            copula: np.ndarray = build_copula_density(
                rank_u, rank_v, grid_size=GRID_SIZE, sigma=1.0
            )

            # ch1: Conditional density P(rank_j | rank_i) — row-normalized
            cond_density: np.ndarray = build_conditional_density(copula)

            # ch2: ANM copula — scatter of (rank_i, rank_resid_j)
            # Use pairwise ANM (regress j on i only, not all variables)
            resid_j: np.ndarray = compute_anm_residuals_pairwise(
                data[:, i], data[:, j], sub_idx, bandwidth=0.5
            )
            rank_resid: np.ndarray = rank_transform(resid_j)
            anm_copula: np.ndarray = build_copula_density(
                rank_u, rank_resid, grid_size=GRID_SIZE, sigma=1.0
            )

            # Stack 3 channels: (3, H, W)
            edge_img: np.ndarray = np.stack(
                [copula, cond_density, anm_copula], axis=0
            ).astype(np.float32)

            edges.append(edge_img)
            edge_types.append(_edge_type(u_name, v_name))

    edge_data: np.ndarray = np.stack(edges, axis=0)  # (E, 3, H, W)
...
def collate_fn(batch: list[dict]) -> dict:
    ...
    # 2D: (B, max_E, C, H, W)
    edge_data: torch.Tensor = torch.zeros(
        B, max_E, N_2D_CHANNELS, GRID_SIZE, GRID_SIZE
    )
...
# ============================================================
# Model Architecture — Conv2d backbone, rest identical to v11
# ============================================================
class ConvBlock2D(nn.Module):
    def __init__(self, d: int, kernel_size: int = 3, n_groups: int = 8):
        super().__init__()
        self.conv = nn.Conv2d(d, d, kernel_size, padding=kernel_size // 2, bias=False)
...
class EdgeFeatureExtractor2D(nn.Module):
    """Conv2d backbone for 2D density images.

    Input: (B*E, C, H, W) where C=3, H=W=32
    Output: (B*E, d)
    """
    def __init__(self, d: int = 64, n_blocks: int = 5, n_channels: int = 3):
        super().__init__()
        self.stem = nn.Conv2d(n_channels, d, 3, padding=1)
        self.blocks = nn.Sequential(*[ConvBlock2D(d) for _ in range(n_blocks)])
        self.pool = nn.AdaptiveAvgPool2d(1)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = self.stem(x)       # (B*E, d, H, W)
        x = self.blocks(x)     # (B*E, d, H, W)
        x = self.pool(x)       # (B*E, d, 1, 1)
        return x.squeeze(-1).squeeze(-1)  # (B*E, d)
...
class ADIAModel(nn.Module):
    """v23: 2D copula density backbone. Conv2d replaces Conv1d.
    Everything after the extractor (merge, attention, heads) is identical to v11.
    """
    ...
    def __init__(self, d: int = None, n_edge_types: int = None, aug_noise_std: float = 0.0):
        ...
        # v23: Conv2d backbone instead of Conv1d
        self.extractor = EdgeFeatureExtractor2D(d, n_channels=N_2D_CHANNELS)
    ...
    def forward(self, edge_data, edge_types, edge_mask, cols_list, struct_rel=None):
        ...
        # Flatten batch and edges for conv2d
...
class ADIAModelWrapper(pl.LightningModule):
    ...
    def _compute_loss(self, batch: dict, split: str) -> torch.Tensor:
        ...
        if "edge_labels" in batch:
            ...
            flat_logits = edge_logits[mask]
            flat_labels = el[mask]
            if flat_logits.numel() > 0:
                edge_loss = self.edge_criterion(flat_logits, flat_labels)
        ...
        if "node_labels" in batch:
            ...
            if all_node_logits:
                cat_logits = torch.cat(all_node_logits, dim=0)
                cat_labels = torch.cat(all_node_labels, dim=0)
                valid = cat_labels >= 0
                if valid.any():
                    node_loss = self.node_criterion(cat_logits[valid], cat_labels[valid])
...
def train(
    ...
    cache_path: str = os.path.join(
        LOCAL_CACHE_DIR, f"train_dataset_v23_2d_nk{N_KERNEL}.pkl"
    )
    if os.path.exists(cache_path):
        ...
        print(f"Building 2D dataset ({len(args)} samples, {n_workers} workers)...")
    ...
    path: str = os.path.join(model_directory_path, "model_v23.pt")
...
def infer_batch_local(dfs, model, device="cpu", batch_size=32, cache_dir=None):
    model = model.eval()
    cache_path = os.path.join(cache_dir, f"infer_v23_2d_nk{N_KERNEL}.pkl") if cache_dir else None
    if cache_path and os.path.exists(cache_path):
        ...
        # print(f"Building 2D edge tensors ({len(dfs)} graphs, {n_workers} workers)...")
        # ctx = mp.get_context('fork')
        # with ProcessPoolExecutor(max_workers=n_workers, mp_context=ctx) as pool:
...
def infer(X_test, model_directory_path, id_column_name, prediction_column_name):
    path = os.path.join(model_directory_path, "model_v23.pt")
...
if __name__ == "__main__":
    print("=" * 60)
    print("v23: 2D Copula Density + Conv2d")
    print(f"  3ch x {GRID_SIZE}x{GRID_SIZE}: copula, conditional, ANM copula")
    print(f"  Conv2d backbone replaces Conv1d entirely")
    print(f"  Config: bs={BATCH_SIZE}, lr={LR}, epochs={MAX_EPOCHS}")
    print(f"  REQUIRES NEW CACHE")
    print("=" * 60)
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = ADIAModel(d=D_MODEL, aug_noise_std=0.0)
    model.load_state_dict(torch.load("resources/model_v23.pt", map_location=device, weights_only=True))
    model.to(device).eval()
    dfs = [X_test[n] for n in X_test]
    ...
    print(f"\nv23 Balanced Accuracy: {np.mean(accs):.4f}")